...
findComplex.prototype.sqlSubquery = function (params) {
  ...
  if (params.method == 'LATERAL') {
    if (params.sublevels == 0) {
      ...
      switch (params.type) {
        ...
        case 'hasOne':
        case 'belongsTo':
          lateralWhere = ' WHERE ' + params.alias + '.' + params.keyTo + ' = ' + params.parentAlias + '.' + params.keyFrom;
      }
    }
  } else {
    if (params.sublevels == 0) {
  ...
  if (params.method == 'PAGING') {
    if (typeof params.groupBy != 'undefined' && params.groupBy != null) {
      ...
      params.select = params.alias + '.' + params.keyTo;
    }
  }

  if (params.method == 'LATERAL') {
    let wholeObj = 'row_to_json(' + params.alias + ')::jsonb';
    ...
    if (params.type == 'hasManyThrough' || params.type == 'hasMany') {
      sql.sql = sql.sql + 'SELECT jsonb_agg(' + wholeObj + ') AS ' + params.alias + ' FROM (';
    } else {
      sql.sql = sql.sql + 'SELECT ' + wholeObj + ' AS ' + params.alias + ' FROM (';
    }
  }

  sql.sql = sql.sql + 'SELECT ' + params.select + ' FROM ' + params.from_table + ' AS ' + params.alias;

  if (params.method == 'LATERAL') {
    sql.sql = sql.sql + ' ) ' + params.alias;
  }

  sql.sql = sql.sql + join + lateralWhere
  ...
  keys.forEach(function (alias) {
    ...
    tmp.forEach(function (subJoin, index) {
      if (index == 0) {
        ...
        switch (subJoin.relationData.type) {
          case 'hasManyThrough':
          case 'hasMany':
            if (parentJoin && join[alias]) {
              children = join[alias].flatMap(t => Object.keys(t));
            } else if (join[subJoin.relation]) {
              children = join[subJoin.relation].flatMap(t => Object.keys(t));
            }
          case 'hasOne':
          case 'belongsTo':
            if (join[alias]) {
    ...
    if (closeJoin.parentJoin.sublevels == closeJoin.sublevel + 1 && closeJoin.level != 1) {
      ...
      if (method == 'LATERAL') {
        ...
        // @@ Esta mesma condição já aplicada na subquery
        // @@ Além de que isto causa um erro em algumas situações
...
findComplex.prototype.createJoinSelect = function (join, childs) {
  ...
  if (join.select !== false) {
    ...
    sqlSelect = tmpSelect.join(',');

    switch (join.relationData.type) {
      case 'hasMany':
        if (join.level > 0 && !tmpSelect.includes(join.relationData.keyTo)) {
          sqlSelect = sqlSelect + ',' + join.alias + '.' + join.relationData.keyTo + ' AS ' + join.relationData.keyTo;
        }
        break;
      case 'hasManyThrough':
        if (join.level > 0 && !tmpSelect.includes(join.relationData.keyTo)) {
          sqlSelect = sqlSelect + ',' + join.relationData.modelThrough + '.' + join.relationData.keyTo + ' AS ' + join.relationData.keyTo;
        }
        break;
      case 'hasOne':
      case 'belongsTo':
